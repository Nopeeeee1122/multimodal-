...
def read_image():
    ...
    for path in file_list:
        data_transforms = transforms.Compose([   # 添加数据变换
...
def get_text_dict(path):
    text_dict = pickle.load(open(path, 'rb'))
    return text_dict
...
def write_data(flag, image, text_only):
    def read_post(flag):
        ...
        for k, f in enumerate(file_list):
            entity_dict = get_text_dict(f.replace('rumor.txt', 'rumor_ent.pkl'))           
